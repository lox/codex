...
async fn run_ratatui_app(
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        ...
        match RolloutRecorder::list_conversations(&config.codex_home, 1, None, None).await {
            Ok(page) => page
                .items
                .first()
        ...
        match resume_picker::run_resume_picker(&mut tui, &config.codex_home, Some(&config.cwd))
            .await?
        {
